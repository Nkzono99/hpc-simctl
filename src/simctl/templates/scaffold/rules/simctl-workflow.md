...
- `Rxxxx/submit/job.sh` は手書きしない (simctl が生成する)
- run は必ず `simctl runs create` または `simctl runs sweep` で生成する
- `work/` の出力は読み取り専用扱い (移動・削除しない)
- `.simctl/knowledge/` の自動生成物は手で整形しない
- `.simctl/insights/` と `.simctl/facts.toml` は直接編集せず、
  `simctl knowledge save` / `simctl knowledge add-fact` を使う
- `SITE.md` は site profile 由来の生成ドキュメントとして直接編集しない
- `refs/` 配下は外部リポジトリのミラーなので書き込まない
- `runs/**/input/*` を緊急修正した場合は、同じ修正を上流の case へ戻す

## tools/hpc-simctl の編集

- `tools/hpc-simctl/` は editable install されているので、必要なら
  ソースを直接編集して即試せる (settings.json でも allow になっている)
- 修正は基本 `simctl` 本体側で先に行い、commit / push してから
  プロジェクト側に反映する
...
- **case は `simctl case new <name> -s <simulator>` で生成する**
  (`cases/<sim>/` に自動配置)
- 生成された `case.toml` や入力テンプレートの編集は自由

## ジョブ投入の承認フロー

`simctl runs submit` は permissions.ask で毎回確認が入る (フックは廃止
...
`simproject.toml`, `simulators.toml`, `launchers.toml`, `CLAUDE.md`,
...
- `simctl runs sweep` で新しい run を生成したとき
- 解析結果・知見を保存したとき
- `tools/hpc-simctl/` を修正してテストが通ったとき
- `simctl runs submit` の前 (投入前のスナップショット)
