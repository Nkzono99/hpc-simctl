# simctl へのフィードバック (upstream feedback)

このプロジェクトは `hpc-simctl` (https://github.com/Nkzono99/hpc-simctl) 上で
運用されている。Agent は日常運用の過程で simctl 自体の改善点
(バグ・使いにくさ・不足している機能・分かりにくい docs / エラーメッセージ・
抜けている adapter / launcher サポート 等) に気づいたら、upstream に
フィードバックして simctl を育てることもタスクに含める。

## いつフィードバックするか

- `simctl <command>` がクラッシュした、または unhelpful なエラーで止まった
- コマンド体系や help が実態と噛み合っていない
- 「もう一段抽象化してあれば手作業が減る」ワークフローが繰り返し出てきた
- `CLAUDE.md` / skill / rule の記述と実装が食い違っている
- 新しい simulator / launcher / site を足したくなった
- `tools/hpc-simctl/` のコードを読んでいて明らかなバグ・dead code を発見した

「この 1 回だけ回避できればよい」ではなく、**次に別のプロジェクトで同じ作業を
する人 (自分含む) が困らないようにする** という視点で判断する。

## フィードバック手順

1. **まずユーザーに提案する** — issue を切る前に必ず「こういう点で
   simctl に issue を立てようと思うがどうか」と相談する。勝手に issue を
   投げない。
2. ユーザーが OK したら、`.venv` を activate した状態で `gh` を使って
   issue を作成する:

   ```bash
   gh issue create \
     --repo Nkzono99/hpc-simctl \
     --title "<短い要約>" \
     --body "$(cat <<'EOF'
   ## 概要
   <何が問題か / どんな改善を提案するか>

   ## 再現手順 (bug の場合)
   1. ...
   2. ...

   ## 期待する挙動
   <どうあるべきか>

   ## 実際の挙動
   <いま何が起きているか / エラーメッセージ>

   ## 環境
   - simctl version: $(simctl --version 2>/dev/null || echo unknown)
   - OS: $(uname -srm)
   - simulator: <該当 adapter 名 / バージョン>
   - 関連 run: <run_id があれば>

   ## 補足
   <ログ抜粋, 関連ファイルパス, 既に試した workaround など>
   EOF
   )"
   ```

3. 投げた issue の URL を `notes/YYYY-MM-DD.md` (`simctl notes append`) に
   記録しておく。後から「あのとき投げた issue」で辿れるようにする。

## 軽微な修正は直接 PR でもよい

`tools/hpc-simctl/` は editable install されているので、typo 修正や
small な挙動改善は以下の手順で直接 upstream に投げられる:

1. `cd tools/hpc-simctl && git checkout -b fix/<short-name>`
2. 修正を入れて `uv run pytest` などでテストが通ることを確認
3. プロジェクト側の再現環境で実挙動を確認
4. `git push origin fix/<short-name>` → `gh pr create --repo Nkzono99/hpc-simctl`
5. 作成した PR の URL をユーザーに共有し、承認を求める

大きな変更 (API 変更, 新 adapter, ワークフロー変更) は PR の前に issue で
先に設計を合意する。

## しないこと

- ユーザーに相談せずに issue や PR を作らない
- 同じ内容の issue が既にあるか確認せずに重複 issue を切らない
  (`gh issue list --repo Nkzono99/hpc-simctl --search "<keyword>"` で検索)
- プロジェクト固有の private 情報 (実データパス, クラスタ固有の秘密など)
  を issue 本文に貼らない
- フィードバックを **理由にして現プロジェクトの作業を止めない** — issue や
  PR は作業のサイドチャネルとして投げ、当面の研究タスクは workaround で
  進める
