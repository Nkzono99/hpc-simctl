...
`.simctl/insights/` と `.simctl/facts.toml` は **curated knowledge** (整理済の
...
| 場所                          | 用途                                     | 性質                |
| ---                           | ---                                      | ---                 |
| `.simctl/facts.toml`          | 機械可読 atomic claim                    | curated, atomic     |
| `.simctl/insights/<name>.md`  | 名前付き整理済知見 (`/learn` で書く)     | curated, durable    |
...
- **`simctl notes append "<title>" "<body>"`** — 今日の `notes/YYYY-MM-DD.md`
...
- **`/note` skill** — agent から呼んで note を append (内部で `simctl notes append` を呼ぶ)

## なぜ `simctl knowledge save` ではダメか
...
notes/reports/<topic>.md     ← 整理済 long form report
        ↓ (atomic な知見を抽出)
.simctl/insights/<name>.md   ← 名前付き curated insight
.simctl/facts.toml           ← 機械可読 atomic claim
```
