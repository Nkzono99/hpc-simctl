---
name: create-run
description: Create runs or expand a survey from cases. Use when generating simulation runs with simctl runs create or simctl runs sweep.
---
...
# 生成先ディレクトリへ移動して実行
cd runs/test/basic
simctl runs create <case_name>

# または --dest で生成先を指定
simctl runs create <case_name> --dest runs/test/basic
```
...
# run 一覧を表示
simctl runs list
simctl runs list runs/sheath/angle_scan
...
simctl runs submit -qn <partition>

# survey 全体を投入 (/run-all スキル推奨)
cd runs/sheath/angle_scan
simctl runs submit --all -qn <partition>
```

## 注意

- run ディレクトリを手で作らない (必ず `simctl runs create` / `simctl runs sweep` を使う)
