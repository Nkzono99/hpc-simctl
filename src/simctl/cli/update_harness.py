...
``simctl update-harness`` re-renders all agent-harness templates (CLAUDE.md,
AGENTS.md, .claude/skills/, .claude/rules/, .claude/settings.json, etc.)
from the current version of simctl and writes them into the project.

Collision detection:  If the on-disk file matches the hash recorded in
``.simctl/harness.lock``, it is assumed to be unedited and is silently
...
def _pull_tools_repo(project_dir: Path) -> str | None:
    """``git pull`` the ``tools/hpc-simctl`` clone.

    Returns:
        Short status message, or ``None`` if the repo does not exist.
    """
    simctl_dir = project_dir / "tools" / "hpc-simctl"
    if not (simctl_dir / ".git").is_dir():
        return None
...
def _get_knowledge_imports_path(project_dir: Path) -> str:
    ...
    imports_file = project_dir / ".simctl" / "knowledge" / "enabled" / "imports.md"
    if imports_file.is_file():
        return ".simctl/knowledge/enabled/imports.md"
    return ""
...
def update_harness(
    ...
    skip_pull: Annotated[
        bool,
        typer.Option("--skip-pull", help="Skip 'git pull' on tools/hpc-simctl."),
    ] = False,
    only: Annotated[
        Optional[str],
        typer.Option(
    ...
    """Re-render harness files from the current simctl templates.
    ...
    Examples:
      simctl update-harness                   # update all harness files
      simctl update-harness --dry-run         # preview changes
      simctl update-harness --force           # force-overwrite everything
      simctl update-harness --adopt           # lock current state
      simctl update-harness --only CLAUDE.md  # update a single file
    """
    ...
    try:
        ...
        typer.echo("No simproject.toml found. Are you inside a simctl project?")
        raise typer.Exit(code=1) from None

    # Pull tools/hpc-simctl
    if not skip_pull and not dry_run:
        pull_status = _pull_tools_repo(project_dir)
        if pull_status is not None:
            typer.echo(f"tools/hpc-simctl: {pull_status}")
