...
0. **venv を activate する** — `source .venv/bin/activate` (Linux/Mac) / `.venv\Scripts\activate` (Windows)
   runops や関連パッケージは `.venv/` にインストール済み。activate しないと runops コマンドが使えない。
1. `runops context --json` で project の現在地を把握する
2. `campaign.toml` を読む
3. 関連する `cases/**/case.toml` と `runs/**/survey.toml` を読む
4. `.runops/facts.toml` を読み、必要なら `.runops/knowledge/candidates/facts/` も確認する
5. 必要なら最近の failed run の `runops runs status` / `runops runs log -e` を見る
...
| `/feedback-runops` | runops 本体へのバグ報告・機能要望を issue 化 (引数なしで候補一覧) |
| `/release` | プロジェクトのマイルストーンタグを作成 |
| `/setup-env` | 環境セットアップ |
...
- `tools/runops/` のソース修正 (editable install されているので即試せる)

### 確認が必要なもの
- 初回の大規模 survey / `runops runs submit --all` / 資源増加を伴う retry
- `runops runs purge-work` / `runops runs delete` (実ファイル削除)
- 実行バイナリ、モジュール、launcher の変更

### 報告してから進めるもの
- `runops runs cancel` (実行中の job を停止するが、追加確認プロンプトは不要)
...
- `runops runs sweep` で新しい run を生成したとき (run 設定が確定したとき)
- 解析結果・知見を `.runops/insights/` や `.runops/facts.toml` に保存したとき
- `tools/runops/` を修正してテストが通ったとき
- `runops runs submit` の前 (投入前のスナップショットとして)
...
| 種類 | 性質 | 書き先 | スキル |
|---|---|---|---|
| 整理済の名前付き知見 | curated, durable | `.runops/insights/<name>.md`, `.runops/facts.toml` | `/learn` |
...
- 共有 source 由来の candidate fact を採用するときは
  `runops knowledge promote-fact` で local fact に昇格する
...
1. `runops notes list` で最近の lab notebook の日付を確認
2. 当該テーマに関連する `notes/YYYY-MM-DD.md` を `runops notes show` で読む
3. 散らばっている観察・仮説・反例を一つの insight にまとめる
4. `runops knowledge save` / `add-fact` で永続化する
...
`/learn` で `.runops/insights/` / `facts.toml` に昇格する。
...
1. `.claude/skills/` → simulator docs / enabled knowledge → `tools/runops/docs/`
2. `runops --help` / `runops <command> --help`
3. `tools/runops/src/` は最後の手段
{% if doc_repos %}
