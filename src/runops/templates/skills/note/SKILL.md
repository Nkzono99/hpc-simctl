...
| 整理済の **名前付き知見** | `/learn` | `.runops/insights/<name>.md` | curated, durable, 上書き可 |
| 機械可読な **atomic claim** | `/learn` | `.runops/facts.toml` | curated, atomic |
...
- 整理済の永続知見 → `/learn` で `.runops/insights/`
- 1 つの atomic claim → `/learn` で `.runops/facts.toml`
...
2. さらに永続知見にしたい部分は `/learn` で `.runops/insights/` / `facts.toml` に昇格
