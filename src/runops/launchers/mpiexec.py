...
The most portable MPI launcher, using the standard ``mpiexec`` command.
When use_slurm_ntasks is True, the task count is expressed as
``${SLURM_NTASKS}`` in the exec line for shell expansion.
"""
...
class MpiexecLauncher(Launcher):
    ...
    Attributes:
        n_flag: The flag used to specify the number of processes
            (default ``"-n"``).
    """
    ...
    def build_launch_command(
        ...
        Args:
            ...
            extra_options: Additional mpiexec CLI options (key-value pairs).
        ...
        parts: list[str] = [self.command, self._n_flag, str(ntasks)]
    ...
    def build_exec_line(
        ...
        Args:
            ...
            extra_options: Additional mpiexec CLI options.
        ...
        n_str = "${SLURM_NTASKS}" if self.use_slurm_ntasks else str(ntasks)
        ...
        return f"{option_str} {self._n_flag} {n_str} {prog_str}"
