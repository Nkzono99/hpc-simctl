...
| コマンド | 説明 |
|---------|------|
| `simctl init` / `setup` / `doctor` | プロジェクト管理 |
| `simctl case new` / `runs create` / `runs sweep` | case / run 生成 |
| `simctl runs submit [--all]` | ジョブ投入 |
| `simctl runs status` / `sync` / `log` / `dashboard` | モニタリング |
| `simctl analyze summarize` / `collect` | 解析 |
| `simctl notes append` / `knowledge save` | 知見管理 |
| `simctl runs archive` / `purge-work` / `cancel` / `delete` | ライフサイクル |
