...
hpc-simctl の知識層は 4 つのドメインで構成される:
...
```
project/
  simproject.toml                # プロジェクト設定 ([knowledge] セクション含む)
  campaign.toml                  # 研究意図
...
| **local knowledge** | project 固有の知識 | `.simctl/insights/`, `.simctl/facts.toml` 等 |
| **derived knowledge** | source と local から生成される派生物 | `.simctl/knowledge/enabled/` |
...
`simctl init` 時に CLAUDE.md テンプレートに自動挿入される。
