...
| ドメイン | 内容 | 管理場所 | 更新契機 |
|----------|------|----------|---------|
| **シミュレータ知識** | パラメータスキーマ、物理的制約、安定性条件 | `refs/`, `.runops/knowledge/` | `runops update-refs` |
| **外部共有知識** | ラボ共通の解析手法・知識、シミュレータ汎用知識 | `refs/knowledge/` | `runops knowledge source sync` |
| **実行環境知識** | クラスタ構成、パーティション、モジュール | `.runops/environment.toml` | `runops doctor` |
...
project/
  ...
  notes/                         # Lab notebook (chronological, append-only)
    2026-04-08.md                # 日次の作業ノート (`runops notes append`)
    2026-04-09.md
...
`runops init` 時に各 Adapter の `doc_repos()` で指定されたリポジトリを `refs/` にクローンする。シミュレータのソースコード、ドキュメント、スキーマが含まれる。

```bash
# 更新
runops update-refs          # 全シミュレータ
runops update-refs emses    # 特定のシミュレータのみ
runops update-refs --dry-run
```
...
Auto-generated by `runops update-refs` on 2026-03-30
...
```
$ runops runs create flat_surface
...
`runops doctor` は `sinfo` と `module list` を使ってパーティション情報とロード済みモジュールを自動検出する。
...
runops knowledge source attach path previous-campaign ../previous-campaign --kind project
runops knowledge source attach path shared-insights ../shared-insights --kind insights

# 同期 (git clone/pull + imports.md 再生成)
runops knowledge source sync                    # 全ソース
runops knowledge source sync shared-kb          # 特定ソースのみ
...
# profile の切替
runops knowledge profile enable shared-kb common-analysis
runops knowledge profile disable shared-kb emses-basic
...
`runops knowledge source render` が `.runops/knowledge/enabled/imports.md` を生成する。
このファイルは有効な profile への `@import` 参照を含み、
project の `CLAUDE.md` から `@.runops/knowledge/enabled/imports.md` で一括参照できる。

`runops init` 時に CLAUDE.md テンプレートに自動挿入される。

### init / setup での動作

- **`runops init`** (対話モード): GitHub の `*shared_knowledge*` リポジトリを自動検索し、候補として提案。手動 URL 入力も可能
- **`runops setup`**: `runops.toml` に設定された知識ソースを自動同期し、`imports.md` をレンダリング
...
`runops knowledge save <name>` は **同名で書くと上書き** される。
...
| 用途 | 場所 | 性質 | コマンド |
|---|---|---|---|
| 整理済の名前付き知見 | `.runops/insights/<name>.md` | curated, durable, 上書き可 | `runops knowledge save` |
| 機械可読 atomic claim | `.runops/facts.toml` | curated, atomic | `runops knowledge add-fact` |
| 日次の lab notebook | `notes/YYYY-MM-DD.md` | append-only, chronological | `runops notes append` |
...
echo "..." | runops notes append "<title>"
...
# 一覧 (新しい順, デフォルト 14 日)
runops notes list
runops notes list -n 30

# 内容表示
runops notes show today      # 今日 (JST)
runops notes show latest     # 最新の日
runops notes show 2026-04-08 # 特定日
```
...
`runops notes list` で日付一覧を取り、関連する `runops notes show` で
読んで、散らばった raw observation を一つの insight にまとめてから
`runops knowledge save` / `add-fact` で curated 化する。出処になった
...
```bash
# 保存
runops knowledge save mag_results -t result -s emses \
  -m "磁場角度 45度でイオンフラックス最大"

# 構造化 fact の追加
runops knowledge add-fact "CFL limit: dt < 1.0 for emses" \
...
runops knowledge facts --scope emses -c high
...
`runops knowledge source sync` の流れ:
...
`runops knowledge promote-fact <source>:<fact_id>` で local `.runops/facts.toml`
へ昇格する。
...
2. PLAN: parameter_schema() + validate_params() を活用
   → 物理的に妥当なパラメータセットを設計
   → 設計の理由・トレードオフ・却下案を runops notes append で残す

3. EXECUTE: runops runs create + runops runs submit
   → survey.toml → run 生成 → job 投入
   → 投入直前に runops notes append でスナップショット (commit hash, run_id 範囲)

4. MONITOR: runops runs status + runops runs log + runops runs sync
   → 実行状況の追跡 (bulk sync は terminal state を silent skip)
   → 異常があれば runops notes append で記録

5. ANALYZE: runops analyze summarize + runops analyze collect
   → 結果の集計・要約
   → 観察・仮説を runops notes append で raw に残す

6. LEARN: runops notes list / show で素材を集めて runops knowledge save / add-fact
