...
### simctl case new でテンプレート生成

`simctl case new` コマンドでケーステンプレートを自動生成できます。

```bash
# cases/<simulator>/ ディレクトリ内で実行
cd cases/emses
simctl case new solar_wind_flat

# survey.toml のスタブも同時に生成する場合
simctl case new solar_wind_flat --survey
```
...
```bash
simctl runs create cavity_base --dest runs/cavity/test
```
...
`simctl case new --survey` でケースと同時にスタブを生成するか、手動で作成します。

```bash
# 方法 1: simctl case new --survey で自動生成済みの場合
...
```bash
simctl runs sweep runs/cavity/rectangular/u_aspect_scan
```
...
```bash
simctl runs submit R20260327-0001
```
...
```bash
simctl runs submit runs/cavity/rectangular/u_aspect_scan/R20260327-0001
```

### survey 内の全 run を一括投入

```bash
simctl runs submit --all runs/cavity/rectangular/u_aspect_scan
```
...
```bash
simctl runs submit --all runs/cavity/rectangular/u_aspect_scan --dry-run
```
...
# 特定ディレクトリ以下の run
simctl runs list runs/cavity/rectangular/u_aspect_scan

# 状態でフィルタ
simctl runs list --status failed

# タグでフィルタ
simctl runs list --tag production
```
...
```bash
simctl analyze summarize R20260327-0001
```

Adapter が出力ファイルを解析し、`analysis/summary.json` を生成します。
`simctl case new` で生成される `cases/<simulator>/<case>/summarize.py` があれば続けて実行され、追加メトリクスや `analysis/figures/` 配下の可視化も同時に保存できます。
...
```bash
simctl analyze collect runs/cavity/rectangular/u_aspect_scan
```
...
```bash
simctl analyze plot runs/cavity/rectangular/u_aspect_scan --list-columns
```

そのうえで、sweep 軸と観測量を指定して図を生成します:

```bash
simctl analyze plot runs/cavity/rectangular/u_aspect_scan \
  --x param.aspect \
  --y potential_final_v
```
...
```bash
simctl runs archive --yes R20260327-0001
```

### work/ の不要ファイル削除

```bash
simctl runs purge-work --yes R20260327-0001
```
...
# パスで指定
simctl runs status runs/cavity/rectangular/u_aspect_scan/R20260327-0001
```
