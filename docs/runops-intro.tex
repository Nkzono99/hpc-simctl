...
runops は Claude Code 用のハーネスを自動生成します。\texttt{runops update-harness} コマンドにより以下が配置されます:
...
\begin{itemize}[nosep]
  \item \textbf{runops runs submit} (ジョブ投入): 対象 run・queue・資源量を提示してから実行
  \item \textbf{runops runs delete / purge-work} (実ファイル削除)
...
\begin{itemize}[nosep]
  ...
  \item \textbf{runops update-harness} でハーネスファイル (CLAUDE.md, skills, rules 等) を再生成
  \item \textbf{runops update} でシミュレータパッケージを更新
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{runops 初期化} --- \texttt{uvx --from runops runops init \&\& source .venv/bin/activate}
