"""Tests for the ``runops runs log`` CLI command."""
...
from runops.cli.log import _find_latest_log, _get_progress, _tail_file
...
def test_get_progress_formats_summary_from_adapter(tmp_path: Path) -> None:
    ...
    with patch("runops.adapters.registry.get", return_value=fake_adapter_cls):
...
def test_get_progress_returns_none_when_adapter_lookup_fails(tmp_path: Path) -> None:
    with patch("runops.adapters.registry.get", side_effect=RuntimeError("missing")):
...
def test_runs_log_reads_stderr_with_fallback_pattern(tmp_path: Path) -> None:
    ...
    with patch("runops.cli.log._get_progress", return_value=None):
...
def test_runs_log_surfaces_manifest_read_errors(tmp_path: Path) -> None:
    with (
        patch("runops.cli.log.resolve_run_or_cwd", return_value=tmp_path),
        patch("runops.cli.log.read_manifest", side_effect=SimctlError("bad manifest")),
    ):
